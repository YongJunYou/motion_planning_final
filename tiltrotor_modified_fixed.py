...
parser.add_argument("--mode", type=str, default="hover", choices=["hover", "circle", "stand90"])
...
class Ros2CurrentPublisher:
    ...
    def publish(self, robot, thrusts, wrench=None):
...
def find_robot_handles(robot):
    ...
    return root_body_ids, rotor_ids, tilt_joint_ids, rotor_names, tilt_joint_matches
...
def print_periodic_status(t, p_d, R_d, robot, thrusts):
...
def main():
    ...
    wrench_body_ids, rotor_body_ids, tilt_joint_ids, rotor_names, tilt_joint_names = find_robot_handles(robot)
    ...
    if ros_pub is not None:
        ros_pub.publish(robot, torch.zeros(robot_num, 4, device=device), torch.zeros(robot_num, 6, device=device))
    ...
    try:
        while simulation_app.is_running():
            ...
            if ros_pub is not None:
                ros_pub.publish(robot, thrusts, wrench)

            if state.count % 200 == 0:
                print_periodic_status(state.t, p_d, R_d, robot, thrusts)
    finally:
        if ros_pub is not None:
            ros_pub.shutdown()
